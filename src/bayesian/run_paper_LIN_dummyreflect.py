...
# Output storage
run_id = 'PG_paper_dummyreflect-A1'
...
def bayesian_likelihood(params):
    ...
    model1 = Diusst(T_f=ftemp1, kappa=kappa, mu=mu, alpha=alpha, lambd=lambd, sigma=sigma, z_ref=z_ref,
                    wind_max=wind_max, k_mol=k_mol, z_f=z_f, dz0=dz, ngrid=ngrid,
                    CFL=CFL, diffu_profile=diffu_type, reflect=True, surflux=surflux)

    model2 = Diusst(T_f=ftemp2, kappa=kappa, mu=mu, alpha=alpha, lambd=lambd, sigma=sigma, z_ref=z_ref,
                    wind_max=wind_max, k_mol=k_mol, z_f=z_f, dz0=dz, ngrid=ngrid,
                    CFL=CFL, diffu_profile=diffu_type, reflect=True, surflux=surflux)

    # interpolate to meet CFL condition
    data1, dtlist1, idx1 = model1.interpolate(data_orig1)
    data2, dtlist2, idx2 = model2.interpolate(data_orig2)

    data1['atemp'] = data1['atemp'] - data1['ftemp'] + ftemp1
    data2['atemp'] = data2['atemp'] - data2['ftemp'] + ftemp2

    simu1 = model1.simulate(data1)
    simu2 = model2.simulate(data2)
