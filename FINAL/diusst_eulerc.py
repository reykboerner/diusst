...
def diusst_eulerc(
    ...
    for i in range(N_z):
        # diffusivity
        kappa[:,i] = k_mol + k_eddy * (1-k0*np.exp(z[i]/gamma))/(1-k0*np.exp(-z_f/gamma)) * (np.minimum(maxwind, u))**2
        dkappadz[:,i] = - k_eddy*k0/gamma * np.exp(z[i]/gamma) /(1-k0*np.exp(-z_f/gamma)) * (np.minimum(maxwind, u))**2
        #solar radiation
