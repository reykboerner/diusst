...
# Fit parameters
param_names = ['kappa', 'mu', 'alpha', 'sigma', 'lambd']

# Parameter limits
param_min = [0, 0.0005, 0.05, 0, 0.1]
param_max = [5e-4, 0.05, 10, 1, 10]

# Initial walker positions
param_start = [1e-4, 0.006, 4, 0.8, 3]
...
def bayesian_likelihood(params):
    kappa, mu, alpha, sigma, lambd = params
...
def log_prob(x):
    if (x<param_min).any() or (x>param_max).any():
        ...
        log_prior = - (x[1]-mu0)**2/(2*mustd**2) - np.log( (param_max[0]-param_min[0])*(param_max[2]-param_min[2])*(param_max[3]-param_min[3])*(param_max[4]-param_min[4]) )
